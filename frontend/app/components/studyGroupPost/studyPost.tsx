...
const Post = ({ info, studygroup_id }: Props) => {
  ...
  const handleComment = () => {
    console.log("id", info.id, token, studygroup_id)
    addComment({
      postId: info.id,
      data: { content: comment },
      accessToken: token,
      classroomId: studygroup_id,
    });
    setComment("");
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 space-y-4 relative">
      <div className="flex items-center justify-between">
        ...
        <div className="absolute right-0 top-10 bg-white shadow-lg rounded-md p-2 w-40">
          <ul className="space-y-1">
            {info.is_processed ? (
              <Link
                href={{ pathname: "/dashboard/quiz", query: { post_id: info.id, title: info.content } }}
            ...
            {info.is_processed ? (
              <Link
                href={{ pathname: "/dashboard/flashcard", query: { post_id: info.id } }}
                onClick={() => setMenuOpen(false)}
              >
                <li className="hover:bg-gray-100 px-2 py-1 cursor-pointer">
                  Generate Flashcard
      ...
      <div className="font-semibold ml-10">{info.content}</div>
      {info.file && (
        <div className="flex items-center space-x-2 bg-gray-100 p-2 rounded">
          <FileText className="h-5 w-5" />
          <a href={`${BASEURL}${info.file}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
            View Attached File
          </a>
        </div>
            )}
      ...
      {more &&
        info.comments.map((item: any, ind: number) => (
          <Comment key={ind} post_id={info.id} class_id={studygroup_id} info={item} />
        ))}
